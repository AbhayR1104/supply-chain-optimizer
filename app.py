...
# --------------------
# CORRECTED GREEDY SOLVER (NON-OPTIMIZED)
# --------------------
def run_greedy_solver_and_generate_map(all_locations_df, orders_df, warehouses_df, num_vehicles, vehicle_capacity):
    unvisited_orders_indices = list(orders_df.index + len(warehouses_df))
    routes = []
    total_distance = 0

    for _ in range(num_vehicles):
        if not unvisited_orders_indices: break # Stop if all orders are served
        ...
        while True:
            ...
            for order_idx in unvisited_orders_indices:
                ...
                if current_load + demand <= vehicle_capacity:
                    loc1 = all_locations_df.iloc[current_loc_idx]
                    loc2 = all_locations_df.iloc[order_idx]
                    dist = calculate_distance_in_miles(loc1['lat'], loc1['lon'], loc2['lat'], loc2['lon'])
                    if dist < min_dist:
                        min_dist = dist
    ...
    fig = px.scatter_mapbox(orders_df, lat="latitude", lon="longitude", hover_name="Zone",
                          color_discrete_sequence=["blue"], zoom=9, height=600)
    fig.add_trace(px.scatter_mapbox(warehouses_df, lat="Latitude", lon="Longitude",
                                  hover_name="Warehouse_ID", color_discrete_sequence=["red"], size_max=20).data[0])
    ...
    for i, route in enumerate(routes):
        ...
        for j in range(len(route_df) - 1):
            loc1 = route_df.iloc[j]
            loc2 = route_df.iloc[j+1]
            route_dist += calculate_distance_in_miles(loc1['lat'], loc1['lon'], loc2['lat'], loc2['lon'])
        total_distance += route_dist

        fig.add_trace(go.Scattermapbox(mode="lines", lon=route_df['lon'], lat=route_df['lat'],
                                     line=dict(width=2, color=color_scale[i % len(color_scale)]), name=f"Vehicle Route {i}"))
...
# --------------------
# MASTER OPTIMIZATION FUNCTION (OR-TOOLS)
# --------------------
def run_optimization_and_generate_map(num_vehicles, vehicle_capacity):
    # This function is now just for OR-Tools
    # --- 1. DATA PREPARATION ---
    try:
        df_trips = pd.read_parquet('yellow_tripdata_2024-01.parquet')
    except FileNotFoundError: return dcc.Markdown("### Error: `yellow_tripdata_2024-01.parquet` not found.")
...
265,0.000000,0.000000""" 
    ...
    if total_demand > num_vehicles * vehicle_capacity: return dcc.Markdown(f"### Error: Insufficient capacity. Demand: {total_demand}, Capacity: {num_vehicles * vehicle_capacity}")
    all_locations_df = pd.concat([warehouses_df.rename(columns={'Latitude': 'lat', 'Longitude': 'lon'}),
                                  orders_df.rename(columns={'latitude': 'lat', 'longitude': 'lon'})], ignore_index=True)
    # --- 2. SOLVER ---
    global data # Using global to share data with the greedy function
    data = {}
    ...
    data.update({'distance_matrix': distance_matrix, 'demands': [0]*5 + list(orders_df['Total_Demand']),
                 'vehicle_capacities': [vehicle_capacity]*num_vehicles, 'num_vehicles': num_vehicles, 'depot': 0})
    manager = pywrapcp.RoutingIndexManager(len(data['distance_matrix']), data['num_vehicles'], data['depot'])
    routing = pywrapcp.RoutingModel(manager)
    def distance_callback(from_index, to_index): return int(data['distance_matrix'][manager.IndexToNode(from_index)][manager.IndexToNode(to_index)] * 1000)
    ...
    # --- 3. VISUALIZATION ---
    if not solution: return dcc.Markdown("### No solution found.")
    routes = []
    ...
    fig.update_layout(mapbox_style="open-street-map", margin={"r":0,"t":40,"l":0,"b":0})
    return fig, all_locations_df, orders_df, warehouses_df, solution.ObjectiveValue() / 1000
...
app.layout = html.Div([
    ...
    html.Div(className='row', children=[
        ...
        html.Div(className='eight columns', children=[dcc.Loading(dcc.Graph(id='output-map'))])
    ], style={'padding': '20px'}),
...
def update_map(n_clicks, num_vehicles, vehicle_capacity, strategy):
    ...
    # Run the main optimization to get a consistent set of orders and data
    result = run_optimization_and_generate_map(num_vehicles, vehicle_capacity)
    
    # Check for an error message from the optimizer
    if isinstance(result, html.Div) or isinstance(result, dcc.Markdown):
        return go.Figure(layout={'title': 'Error'}), result.children

    optimized_fig, all_locations_df, orders_df, warehouses_df, optimized_dist = result

    if strategy == 'optimized':
        summary = f"Optimized Total Distance: {optimized_dist:.2f} miles"
        return optimized_fig, summary
    
    elif strategy == 'greedy':
        greedy_fig, greedy_dist = run_greedy_solver_and_generate_map(all_locations_df, orders_df, warehouses_df, num_vehicles, vehicle_capacity)
        summary = f"Optimized: {optimized_dist:.2f} miles vs. Non-Optimized: {greedy_dist:.2f} miles"
        return greedy_fig, summary
